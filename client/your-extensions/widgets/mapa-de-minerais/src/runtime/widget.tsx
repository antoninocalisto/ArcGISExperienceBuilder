...
export default function Widget(props: any) {
  ...
  React.useEffect(() => {
    let cancelled = false
    async function run() {
      if (categoria !== 'minerals') return
      if (!mineralAnalise) return
      ...
      try {
        ...
        if (!cancelled) setLoadingMinerais(false)
      }
    }
    run()
    return () => { cancelled = true }
  ...
  React.useEffect(() => {
    ...
    if (!mineralAnalise) return
    if (!Array.isArray(dadosMinerais)) return
  ...
  return (
    <div css={wrapperStyle} ref={rootRef}>
      ...
      {categoria === 'minerals' && (
        <>
          {/* 5 rádios em UM container, posicionados via data-key */}
          ...
          {/* (restante da UI de minerais — busca, lista e agrupadores — pode ficar como já está) */}
        </>
      )}

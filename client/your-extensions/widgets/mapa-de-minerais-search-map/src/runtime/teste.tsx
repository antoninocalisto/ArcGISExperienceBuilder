...
export default function Widget(props: any) {
  ...
  const userLockedInterestRef = React.useRef(false)
  ...
  const onMsg = (ev: MessageEvent) => {
    ...
    if (data.type === 'fetchDistribuicaoAmostras:ok') {
      ...
      //Ajustando intervalo de interesse para aparecer
        setshowWithInterest(prev => prev || !!cfg?.message?.visible)  // só visibilidade
        autoCheckIfFirstOpen(!!cfg?.message?.visible || !!cfg?.startWithInterest)
    ...
    if (data.type === 'legend:minerais' || data.type === 'fetchDistribuicaoMinerais:ok') {
      ...
      //Ajustando intervalo de interesse para aparecer
        setshowWithInterest(prev => prev || !!_msg?.message?.visible) // só visibilidade
        autoCheckIfFirstOpen(!!_msg?.message?.visible)  
 ...
 React.useEffect(() => {
  ...
  const summaryMinerals = React.useMemo(() => {
    ...
    if (!Array.isArray(dadosMinerais) || dadosMinerais.length === 0) {
      return { title: labelFromValue(mineralAnalise as MineralTipo), rows: [] }
    }
    ...
    const title = (withInterest ? '' : '') + labelFromValue(mineralAnalise as MineralTipo)
  ...
  const summaryMineralsAgr = React.useMemo(() => {
    ...
    if (!Array.isArray(agrupadorDados) || agrupadorDados.length === 0) {
      const titleBase =
        agrupador === 'analise' ? 'Quantidade de Análises de ' :
    ...
    const titleBase =
      (agrupador === 'analise' ? `Quantidade de ${mineralSelecionado.nomeMineral}` :
      agrupador === 'media'   ? `Média de ${mineralSelecionado.nomeMineral}` :
                                `Máxima de ${mineralSelecionado.nomeMineral}`)
  ...
  const legendAgr = React.useMemo(() => {
    ...
    const titleBase =
      agrupador === 'analise' ? `Análises de ${mineralSelecionado.nomeMineral}` :
      agrupador === 'media'   ? `Média de ${mineralSelecionado.nomeMineral}` :
                                `Máxima de ${mineralSelecionado.nomeMineral}`
    ...
    if (isPercent) vals = vals.map(v => Math.max(0, Math.min(100, v))) // trava 0..100
  ...
  const toggleTipo = React.useCallback((t: string) => {
    setTiposSel(prev => prev.includes(t) ? prev.filter(x => x !== t) : [...prev, t])
  }, [])

    function autoCheckIfFirstOpen(hasCG: boolean) {
    ...
    <div css={wrapperStyle} ref={rootRef}>
      ...
      {categoria === 'sample' && (
        <>
          ...
          {/* Sumário (aparece quando houver tipos marcados e base) */}
          {summaryGroups.length > 0 ? (
            summaryGroups.every(g => g.rows.length === 0) ? (
              <div css={emptyStateStyle}>
                Não há dados suficientes para gerar os gráficos de amostras com os filtros atuais.
              </div>
            ) : (
              <div css={summaryListStyle}>
                {summaryGroups.map(group => (
                  <React.Fragment key={group.tipo}>
                    ...
                    {group.rows.length === 0 && (
                      <div css={emptyStateStyle}>
                        Sem dados suficientes para <strong>{group.tipo}</strong>.
                      </div>
                    )}
                  </React.Fragment>
                ))}
            ...
            <div css={emptyStateStyle}>Selecione ao menos um tipo de amostra acima.</div>
          )}

          {/* Rodapé (amostras) */}
          <div css={footerDockStyle}>
            <div css={footerStyle}>
      ...
      {categoria === 'minerals' && (
        <>
          ...
          {loadingMinerais && <div style={{ marginBottom: 8 }}>Carregando análises…</div>}
          ...
          <div css={[afterRadiosColStyle, chartsCompact]}>
            <div css={scrollAreaStyle}>
              {summaryMinerals && (
                summaryMinerals.rows.length > 0 ? (
                  ...
                  <div css={emptyStateStyle}>
                    Não há dados suficientes para gerar o gráfico de <strong>{summaryMinerals.title}</strong>.
                  </div>
                )
              )}
            ...
            {legendAgr && (
              legendAgr.max > 0 ? (
                <div css={[legendSpacingFix, metricsCompact]}>
                  <GradientLegend
                ...
                (mineralSelecionado && agrupador) && (
                  <div css={[emptyStateStyle, metricsCompact]} style={{ marginTop: 8 }}>
                    Não há dados suficientes para gerar a legenda/escala de <strong>{legendAgr.title}</strong>.
                  </div>
                )
              )
            )}
